...
function u8ArraySetEnum(array, values, options) {
    u8ArraySetNumber(array, Object.assign(Object.assign({}, options), { value: values.indexOf(options.value) }));
}
...
const plantGridOffsetsThatMustBeFree = (() => {
    const allGridOffsets = flatten([-1, 0, 1].map((row) => [-1, 0, 1].map((col) => ({ row, col }))));
    const predicates = {
...
const state = {
    ...
    grid(row, col) {
        ...
        const cell = {
            ...
            get plant() {
                if (u8ArrayGetNumber(lastMemory(), {
...
function updatePlantSummary(cell) {
    var _a, _b;
    typeDisplay.textContent = ((_a = cell.plant) === null || _a === void 0 ? void 0 : _a.type) || "None";
    growthLevelDisplay.textContent = `${((_b = cell.plant) === null || _b === void 0 ? void 0 : _b.growth) || 0}`;
...
function showTutorialMessage() {
    neutralMessage("You are the black dot. Click on an adjacent grid cell to move to it.<br />" +
...
function showReapSuccess() {
    ...
    if (cell && cell.plant) {
        ...
        failMessage("Reaped some plant somewhere somehow. " +
...
function showSowFail() {
    const cell = getCell(state.player.row, state.player.col);
    if (!cell) {
        failMessage("Can't sow while out of bounds. " +
            "You should not even be out of bounds. This is a bug.");
    }
    else if (!state.selectedInventoryPlant) {
        failMessage("You haven't selected anything to sow. " +
            "(Hint: click on a seed type in your inventory.)");
    }
    else if (cell.plant) {
        failMessage("There is already a plant here. " +
        ...
        failMessage("You should have been able to sow. " +
...
function plantHasRoomToGrow(cell) {
    return !!cell.plant &&
        plantGridOffsetsThatMustBeFree[cell.plant.type].every((p) => { var _a; return !((_a = getCell(cell.row + p.row, cell.col + p.col)) === null || _a === void 0 ? void 0 : _a.plant); });
}
...
function distributeNaturalResources() {
    for (let row = 0; row < ROWS; row++) {
        for (let col = 0; col < COLS; col++) {
            const cell = state.grid(row, col);
            cell.water = Math.min(cell.water + Math.floor(Math.random() * 21) + 5, 100);
